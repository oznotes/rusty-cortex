...
pub fn probe_device(devinst: DevInst) -> Option<DeviceInfo> {
    ...
    let mut interfaces = if !driver.eq_ignore_ascii_case("winusb") {
        devinst
            .children()
            .flat_map(|intf| {
...
pub(crate) fn find_device_interface_path(dev: DevInst, intf: u8) -> Result<WCString, Error> {
    ...
    if driver.eq_ignore_ascii_case("winusb") {
        // Whole device bound to WinUSB (non-composite) — open directly.
        ...
        // Composite device — walk child device nodes to find the requested interface.
        // Works for usbccgp, dg_ssudbus (Samsung), and any OEM composite parent driver.
        let child = dev
            .children()
            .find(|i| get_interface_number(*i) == Some(intf))
            .ok_or_else(|| Error::new(ErrorKind::NotFound, "Interface not found"))?;

        let Some(child_driver) = child.get_property::<OsString>(DEVPKEY_Device_Service) else {
            return Err(Error::new(
        ...
        if !child_driver.eq_ignore_ascii_case("winusb") {
            return Err(Error::new(
                ErrorKind::Unsupported,
                format!("Interface driver is {child_driver:?}, not WinUSB"),
            ));
        }
